...
class erm(nn.Module):
    def __init__(self, input_shape, num_classes, hparams):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr = self.hparams['lr'],weight_decay=1e-4)
        self.optimizer_classifier = torch.optim.Adam(self.classifier.parameters(), lr = self.hparams['lr'],weight_decay=1e-4)
        self.optimizer_featurelizer = torch.optim.Adam(self.featurelizer.parameters(), lr = self.hparams['lr'],weight_decay=1e-4)
